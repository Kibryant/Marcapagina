...
export function getHabitRecommendations(insights: Insight[]): Recommendation[] {
  ...
  insights.forEach((insight) => {
    if (insight.type === 'time') {
      if (insight.title.includes('à noite')) {
        recommendations.push({
          ...
          description:
            'Já que você lê melhor à noite, tente colocar um lembrete para as 21:30.',
        });
    ...
    if (
      ...
      insight.title.includes('final de semana')
    ) {
      recommendations.push({
        title: 'Doses diárias',
        description:
          'Tente ler apenas 5 páginas na segunda-feira para manter o hábito vivo após o domingo.',
      });
    }
  });
